...
// Definisikan tipe untuk file state
interface FileWithSize extends File {
    // File sudah memiliki name, size, type dari bawaan TypeScript
}
...
export default function ImportKendaraanPage() {
    ...
    const validateFile = (selectedFile: File): boolean => {
        ...
        if (!isValidType && !isValidExtension) {
            setStatus('error');
            setMessage('Format file tidak valid. Harap unggah file dengan ekstensi .csv, .xlsx, atau .xls');
            return false;
        }

        if (selectedFile.size > 5 * 1024 * 1024) { // Maksimal 5MB
            setStatus('error');
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!file) {
            setStatus('error');
            setMessage('Silakan pilih file CSV atau Excel terlebih dahulu.');
            return;
        }
        ...
        // Contoh dengan fetch:
        // try {
        //   const response = await fetch('/api/import-kendaraan', {
        //     method: 'POST',
        //     body: formData,
        //   });
        ...
        //   if (response.ok) {
        //     setStatus('success');
        //     setMessage(`Berhasil mengimport "${file.name}". Data kendaraan telah diperbarui.`);
        //     handleRemoveFile();
        //   } else {
        ...
        // } catch (error) {
        //   setStatus('error');
        //   setMessage(`Gagal mengimport file: ${error.message}`);
        // } finally {
        //   setIsLoading(false);
        // }

        // Simulasi proses upload (HAPUS INI saat menggunakan API sungguhan)
        setTimeout(() => {
            setIsLoading(false);
            setStatus('success');
            setMessage(`Berhasil mengimport "${file.name}". Data kendaraan telah diperbarui.`);
            handleRemoveFile(); // Reset form setelah sukses
        }, 2000);
    };

    // Download template berdasarkan format yang dipilih
    const handleDownloadTemplate = () => {
        // TODO: Implementasi download template yang sesuai
        // Bisa download file template CSV atau Excel
        alert('Fitur download template akan segera hadir!\n\nFormat yang didukung: .csv, .xlsx, .xls');
    };
